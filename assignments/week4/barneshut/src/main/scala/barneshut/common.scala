...
case class Fork(
  ...
  val massX: Float = {
    if mass == 0 then centerX else quadList.foldLeft(0f){case (sum, quad) => sum + quad.mass * quad.massX} / mass
  }
  val massY: Float = {
    if mass == 0 then centerY else quadList.foldLeft(0f){case (sum, quad) => sum + quad.mass * quad.massY} / mass
  }
